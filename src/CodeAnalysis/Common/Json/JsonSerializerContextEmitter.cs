...
{
    public static readonly DiagnosticDescriptor InvalidEnvelopeDescriptor = new("COMP0002",
        "Invalid Envelope",
    ...
    public static string GetToken(string discriminator)
    {
        ...
        {
            crc ^= b;
            for (var i = 0; i < 8; i++) crc = (crc >> 1) ^ (0xEDB88320 & (uint)-(int)(crc & 1));
        }
    ...
    public static void DiscoverHierarchy(SerializationGraph graph,
                                         ...
                                         Func<ITypeSymbol, bool>? filter = null)
    {
        ...
        if (root == null)
        {
            if (filter == null || filter(candidate)) graph.AddNonPolymorphic(candidate);
            return;
        }
        ...
        while (current != null && !SymbolEqualityComparer.Default.Equals(current, root))
        {
            ...
            if (filter == null || filter(current)) graph.AddPolymorphic(root, parent, current);
        ...
        if (filter == null || filter(root))
            if (!graph.Families.ContainsKey(root))
                graph.Families[root] = new(SymbolEqualityComparer.Default);
    ...
    static void FanOutChildren(SerializationGraph graph,
                               ITypeSymbol root,
                               ITypeSymbol parent,
                               IEnumerable<ITypeSymbol> allTypesInCompilation,
    ...
    static void CollectTypes(INamespaceOrTypeSymbol symbol, List<ITypeSymbol> types)
    {
        ...
        else if (symbol is INamedTypeSymbol type)
        {
            ...
            {
                types.Add(type);
                foreach (var nested in type.GetTypeMembers()) CollectTypes(nested, types);
            }
        }
    }

    public static void AddSerializableTypes(SerializationGraph graph,
                                            ITypeSymbol? type,
    ...
    public static bool IsRelevantType(ITypeSymbol type)
    {
        ...
        if (type is INamedTypeSymbol named && named.IsGenericType &&
            named.TypeArguments.Any(t => t.TypeKind == TypeKind.TypeParameter))
            return false;
    ...
    public static void EmitGeneratedClass(StringBuilder sb,
                                          IEnumerable<ITypeSymbol> allTypes,
    ...
    public static void EmitJsonSerializerOptions(StringBuilder sb,
                                                 string propertyName,
                                                 string methodName,
                                                 string indent = "    ",
                                                 string className = "")
    ...
    static void CollectConcreteDescendants(Dictionary<ITypeSymbol, HashSet<ITypeSymbol>> hierarchy,
                                           ITypeSymbol parent,
                                           HashSet<ITypeSymbol> result)
    {
        if (hierarchy.TryGetValue(parent, out var children))
            foreach (var child in children)
            {
                if (!child.IsAbstract && child.TypeKind != TypeKind.Interface) result.Add(child);
                CollectConcreteDescendants(hierarchy, child, result);
            }
    }

    public static void EmitContext(StringBuilder sb,
                                   string className,
    ...
    public static void EmitAttributesOnly(StringBuilder sb,
                                          string className,
                                          string accessibility,
                                          SerializationGraph graph)
    {
    ...
    public static void EmitRuntimeMembers(StringBuilder sb,
                                          string className,
                                          string accessibility,
                                          SerializationGraph graph,
                                          string optionsPropertyName = "Options",
    ...
    public class SerializationGraph
    {
        ...
        public void AddPolymorphic(ITypeSymbol root, ITypeSymbol parent, ITypeSymbol child)
        {
            ...
            {
                hierarchy = new(SymbolEqualityComparer.Default);
