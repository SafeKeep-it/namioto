...
public class MethodCallGraphGenerator : IIncrementalGenerator
{
        ...
        var symbolDeclarations = context.SyntaxProvider.CreateSyntaxProvider(
                                            static (s, _) =>
                                                s is MethodDeclarationSyntax or ConstructorDeclarationSyntax or
                                                     ...
                                                     EventDeclarationSyntax or EventFieldDeclarationSyntax or
    ...
    static SymbolMetadata? GetMetadata(GeneratorSyntaxContext context, ImmutableHashSet<string> solutionAssemblies)
    {
                ...
                synthesizedKind = synthMethod.Name switch
                                  {
                                      ...
                                      "PrintMembers" when containingIsRecord => "record-printmembers",
                                      "Deconstruct" when containingIsRecord => "record-deconstruct",
                                      "<Clone>$" when containingIsRecord => "record-clone",
                                      ".ctor" when containingIsRecord && synthMethod.Parameters.Length == 1 &&
                                                   SymbolEqualityComparer.Default.Equals(
                                                       synthMethod.Parameters[0].Type,
                                                       symbol.ContainingType) => "record-copy-ctor",
                                      "op_Equality" or "op_Inequality" when containingIsRecord => "record-equality-op",
                                      "get_EqualityContract" when containingIsRecord => "record-equality-contract",
                                      ...
                                      var _ when synthMethod.MethodKind == MethodKind.EventAdd => "event-add",
                                      var _ when synthMethod.MethodKind == MethodKind.EventRemove => "event-remove",
        ...
        var generatedAttr = symbol.GetAttributes()
                                  .FirstOrDefault(a => a.AttributeClass?.Name == "GeneratedCodeAttribute" ||
        ...
        else if (node.SyntaxTree.FilePath.EndsWith(".g.cs") || node.SyntaxTree.FilePath.EndsWith(".generated.cs"))
        {
            isGenerated = true;
            generatorName = "file";
        }
        ...
        if (symbol is IMethodSymbol methodSymbol)
        {
            ...
            {
                parameters.Add((p.Name, p.Type.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat)));
            ...
            {
                isExtension = true;
                extendedType = methodSymbol.Parameters[0].Type.GetDocumentationCommentId() ?? methodSymbol.Parameters[0]
            ...
            if (methodSymbol.MethodKind == MethodKind.Constructor)
            {
                         ...
                         containingType2.DeclaringSyntaxReferences.Length > 0)
                {
                    ...
                    if (typeSyntax is TypeDeclarationSyntax typeDecl && typeDecl.ParameterList != null)
        ...
        else if (symbol is INamedTypeSymbol typeSymbol)
        {
            ...
            if (typeSymbol.IsStatic && typeSymbol.GetMembers().OfType<IMethodSymbol>().Any(m => m.IsExtensionMethod))
                isExtension = true;
                                                     ...
                                                     m.MethodKind == MethodKind.Conversion))
            {
                ...
                var opReturn = op.ReturnType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
            ...
            if (typeSymbol.TypeKind == TypeKind.Delegate && typeSymbol.DelegateInvokeMethod != null)
            {
                ...
                returnType = invoke.ReturnType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
        ...
        {
            returnType = propertySymbol.Type.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
            var accessors = new List<string>();
            if (propertySymbol.GetMethod != null) accessors.Add("get");
            if (propertySymbol.SetMethod != null) accessors.Add(propertySymbol.SetMethod.IsInitOnly ? "init" : "set");
            ...
            {
                signature = $"{returnType} {propertySymbol.Name} {{ {string.Join("; ", accessors)}; }}";
            }
        ...
        {
            returnType = fieldSymbol.Type.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
        ...
        {
            kind = "event";
            returnType = eventSymbol.Type.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
        ...
        foreach (var attr in symbol.GetAttributes())
        {
            ...
            var namedArgs = attr.NamedArguments.Select(a => (a.Key, a.Value.ToCSharpString())).ToList();
                        ...
                        SourceProductionContext context)
    {
        ...
        foreach (var s in symbols)
        {
            ...
            var json = BuildSymbolJson(s, relativePath);
    ...
    static string BuildSymbolJson(SymbolMetadata s, string relativePath)
    {
        ...
        var attrsJson = string.Join(",",
                                    ...
                                    {
                                        var ctorArgsJson =
                                            string.Join(",", a.ConstructorArgs.Select(c => $"\"{Escape(c)}\""));
                                        var namedArgsJson =
                                            string.Join(",",
        ...
        var returnTypeField = s.ReturnType != null ? $",\"returnType\":\"{Escape(s.ReturnType)}\"" : "";
        var paramsField = s.Parameters.Count > 0 ? $",\"parameters\":[{paramsJson}]" : "";
        var containingField = s.ContainingType != null ? $",\"containingType\":\"{Escape(s.ContainingType)}\"" : "";
        ...
        var extendedTypeField = s.ExtendedType != null ? $",\"extendedType\":\"{Escape(s.ExtendedType)}\"" : "";
        var generatedField = s.IsGenerated ? ",\"isGenerated\":true" : "";
        var generatorField = s.GeneratorName != null ? $",\"generatorName\":\"{Escape(s.GeneratorName)}\"" : "";
        var ctorKindField = s.ConstructorKind != null ? $",\"constructorKind\":\"{Escape(s.ConstructorKind)}\"" : "";
        ...
        var synthesizedKindField =
            s.SynthesizedKind != null ? $",\"synthesizedKind\":\"{Escape(s.SynthesizedKind)}\"" : "";
    ...
    {
        public AttributeMetadata(string name, List<string> constructorArgs, List<(string Name, string Value)> namedArgs)
        {
            Name = name;
