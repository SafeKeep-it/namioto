...
public class HttpClientGenerator : IIncrementalGenerator
{
    ...
    private static void Execute(ImmutableArray<Registration> registrations, Compilation compilation, SourceProductionContext context)
    {
        ...
        // Determine project directory from first registration's source file (where the invocation happens)
        ...
        var manifest = GeneratorManifest.LoadOrCreate("Http", projectDirectory, "// <auto-generated by=\"Comptatata.CodeAnalysis.Http\" />");
        ...
        var byOutputPath = distinctRegistrations
            .GroupBy(r =>
            {
                var interfaceType = (INamedTypeSymbol)r.InterfaceType;
                var sourcePath = interfaceType.DeclaringSyntaxReferences.FirstOrDefault()?.SyntaxTree.FilePath ?? r.SourceFilePath;
                return sourcePath;
            })
        ...
        // Save manifest
        manifest.Save("Http", projectDirectory);
    }
