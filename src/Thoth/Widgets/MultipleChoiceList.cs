...
public sealed class MultipleChoiceList : IWidget,
                                          IWidgetWithLayout,
                                          IFocusable,
                                          ...
                                          IEventHandler<KeyPressedInput>
{
    ...
    static readonly Color fallback_checked_foreground = new(46, 160, 67);

    readonly MultipleChoiceListScribe _scribe;
    ...
    public IReadOnlyList<string> SelectedChoiceIds
    {
        get
        {
            var selected = new List<string>(_rows.Count);
            for (var i = 0; i < _rows.Count; i++)
                if (_rows[i].Toggle.IsChecked)
                    selected.Add(_rows[i].Id);
    ...
    public ILayoutCreator GetLayoutCreator() => new MultipleChoiceListLayout();
    ...
    {
        RenderPhaseGuard.ThrowIfActive("MultipleChoiceList.SetChoices");
    ...
    void IEventHandler<KeyPressedInput>.Handle(IEventContext context, in KeyPressedInput e)
    {
        ...
        if (e.Key.Key == ConsoleKey.Spacebar)
        {
            ...
            _rows[_activeRowIndex].Toggle.ToggleChecked();
            ApplyActiveRowStyles();
            context.RaiseEvent(new OnContentChanged());
            context.IsHandled = true;
        }
    ...
    internal void ApplyActiveRowStyles()
    {
        for (var i = 0; i < _rows.Count; i++)
        {
            ...
            _rows[i].Toggle.CheckedForegroundColor = rowForeground
...
public sealed class MultipleChoiceListLayout : ILayoutCreator
{
    public WidgetSizeRequest Measure(in IWidgetWithLayout widget,
                                     in SizeConstraint constraint,
                                     ReadOnlySpan<WidgetSizeRequest> requests)
    {
        var list = widget as MultipleChoiceList
            ?? throw new InvalidOperationException($"{nameof(MultipleChoiceListLayout)} requires {nameof(MultipleChoiceList)}.");
