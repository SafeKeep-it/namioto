...
public class builds_capture_path_for_event_type : IAsyncLifetime
{
    ...
    public Task InitializeAsync()
    {
        ...
        _target = new HandlingWidget { Parent = _inertPanel };
        _inertPanel.AddChild(_target);

        _path = new EventDispatcher().GetCapturePath<OnMouseClick>(_target);
    ...
    [Fact]
    public void excludes_widgets_not_implementing_capture_interfaces() =>
        _path.ShouldNotContain(_inertPanel);

    [Fact]
    public void excludes_widgets_only_implementing_handle_interfaces() =>
        _path.ShouldNotContain(_target);

    [Fact]
    public void includes_capturer() =>
        _path.ShouldContain(_capturer);

    [Fact]
    public void orders_root_to_target() =>
        _path.ShouldBe([_capturer]);
}
