...
public class dock_layout_creator_contract
{
    [Fact]
    ...
    public void measure_accounts_for_child_desired_size()
    {
        ...
        var childDesired = single_child_layout_test_support.MeasureChild(child, new SizeConstraint(8, 4));
    ...
    public void arrange_writes_child_rect_into_span()
    {
        ...
        creator.Arrange(widget, new WidgetSize(widget, creator, new Rect(0, 0, 9, 4)), ReadOnlySpan<WidgetSizeRequest>.Empty, children);

        children[0].Rect.ShouldBe(new Rect(0, 0, 9, 1));
    }

    static Dock create_widget()
    {
        return new Dock
               {
